...
describe 'redis' do
  ...
  it 'should be able to use a namespace with bpop' do
    ...
    @namespaced.blpop('foo', 1).should eq(%w(foo string))
    @namespaced.blpop('foo', 1).should eq(['foo', 'ns:string'])
    @namespaced.blpop('foo').should eq(%w(foo string_no_timeout))
  ...
  it 'should be able to use a namespace with linsert' do
    ...
    @namespaced.lrange('foo', 0, -1).should eq(%w(bar barf barn bart))
  end
  ...
  it 'should be able to use a namespace with lpushx' do
    ...
    @namespaced.lrange('foo', 0, -1).should eq(%w(bar boo))
  end
  ...
  it 'should be able to use a namespace with rpushx' do
    ...
    @namespaced.lrange('foo', 0, -1).should eq(%w(boo bar))
  end

  it 'should be able to use a namespace with setbit' do
    @namespaced.setbit('virgin_key', 1, 1)
    @namespaced.exists('virgin_key').should be_truthy
  ...
  it 'should be able to use a namespace with mget and connection_pool' do
    old = @namespaced
  ...
  it 'should be able to use a namespace with mapped_msetnx' do
    ...
    @namespaced.mapped_msetnx('foo' => '1000', 'bar' => '2000').should be_falsey
    @namespaced.mapped_mget('foo', 'bar').should eq('foo' => '1', 'bar' => nil)
    @namespaced.mapped_msetnx('bar' => '2000', 'baz' => '1000').should be_truthy
  ...
  it 'should be able to use a namespace with hashes' do
    ...
    @namespaced.hkeys('foo').should eq(%w(key key1))
    ...
    @namespaced.hsetnx('foonx', 'nx', 10).should be_truthy
    @namespaced.hsetnx('foonx', 'nx', 12).should be_falsey
  ...
  it 'should properly union two sorted sets with options' do
    ...
    @namespaced.zunionstore('union', %w(sort1 sort2), weights: [2, 1])
  ...
  it 'should properly union two sorted sets without options' do
    ...
    @namespaced.zadd('sort2', 3, 3)
    @namespaced.zadd('sort2', 4, 4)
    @namespaced.zunionstore('union', %w(sort1 sort2))
  ...
  it 'should properly intersect two sorted sets without options' do
    ...
    @namespaced.zinterstore('inter', %w(food color))
  ...
  it 'should properly intersect two sorted sets with options' do
    ...
    @namespaced.zinterstore('inter', %w(food color), aggregate: 'min')
  ...
  it 'should add namespace to sort' do
    ...
    @namespaced.sort('foo', get: ['#', 'value_*']).should eq([%w(1 a), %w(2 b)])
  ...
  it 'should add namespace to pipelined blocks with connection_pool' do
    old = @namespaced
    @namespaced = Redis::Namespace.new(:ns, redis: ConnectionPool.new(size: 2, timeout: 2) { Redis.new db: 15 })
    @namespaced.mapped_hmset 'foo', 'key' => 'value'
    @namespaced.pipelined do |r|
  ...
  if @redis_version >= Gem::Version.new('2.5.0')
    describe 'redis 2.6 commands' do
      ...
      it 'should namespace dump and restore' do
        ...
        expect(@redis.ttl('ns:foo')).to satisfy { |v| (0..1).cover?(v) }
      ...
      it 'should namespace pexpire' do
        @namespaced.set('mykey', 'Hello')
        @namespaced.pexpire('mykey', 60_000).should eq(true)
      end

      it 'should namespace pexpireat' do
        @namespaced.set('mykey', 'Hello')
        @namespaced.pexpire('mykey', 1_555_555_555_005).should eq(true)
      end

      it 'should namespace psetex' do
        @namespaced.psetex('mykey', 10_000, 'Hello').should eq('OK')
  ...
  if @redis_version >= Gem::Version.new('2.7.105')
    describe 'redis 2.8 commands' do
      context 'keyspace scan methods' do
        ...
        if Redis.current.respond_to?(:scan)
          context '#scan' do
        ...
        if Redis.current.respond_to?(:scan_each)
      ...
      context 'hash scan methods' do
        ...
        let(:hash_matching_subset) do
          ...
          hash.select { |k, _v| k[/\Azeta:/] }
        end
        if Redis.current.respond_to?(:hscan)
          context '#hscan' do
        ...
        if Redis.current.respond_to?(:hscan_each)
      ...
      context 'set scan methods' do
        ...
        if Redis.current.respond_to?(:sscan)
          context '#sscan' do
            context 'when supplied :match' do
        ...
        if Redis.current.respond_to?(:sscan_each)
      ...
      context 'zset scan methods' do
        ...
        if Redis.current.respond_to?(:zscan)
          context '#zscan' do
            context 'when supplied :match' do
        ...
        if Redis.current.respond_to?(:zscan_each)
